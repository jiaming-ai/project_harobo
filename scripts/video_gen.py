...
def create_ovmm_env_fn(config,args):
    ...
    if args.split_dataset is not None:
        idx_start = args.split_dataset[0]
        idx_end = args.split_dataset[1]
        dataset.episodes = dataset.episodes[idx_start:idx_end]
    ...
    def play(
        ...
        while ep_idx < env.number_of_episodes:
            ...
            if visualize:
                ...
                images['rgb_detection'] = ob.task_observations['semantic_frame'][:,:,:3]
                ...
                if sm is not None:
                    ...
                    sem_map = vis['semantic_map']

                images['semantic_map_vis'] = sem_map 
                ...
                if ig_vis is not None and 'utility' in ig_vis:
            ...
            if vis_type == 'paper':
                ...
                third_person = cv2.resize(
                    third_person,
                    (640, 640),
                    interpolation=cv2.INTER_NEAREST,
                )
                draw_ob[:,0:640,:] = third_person
...
if __name__ == "__main__":
    ...
    config.habitat.simulator.agents.main_agent.sim_sensors.third_rgb_sensor.height = 640
    config.habitat.simulator.agents.main_agent.sim_sensors.third_rgb_sensor.width = 640
