...
class HeuristicPickPolicy(HeuristicPlacePolicy):
    ...
    def generate_plan(
        ...
        if found is None:
            ...
            self.t_relative_standoff = self.t_relative_back + self.slow_motion_frames
            self.t_relative_grasp = self.t_relative_standoff + self.slow_motion_frames
            self.t_relative_snap_object = self.t_relative_grasp + 1
            self.t_start_pick = np.inf
            if self.verbose:
                print("-" * 20)
    ...
    def get_action(
        ...
        if self.timestep == self.t_start_pick + self.t_relative_back:
            ...
            progress_ratio = (
                self.timestep - self.t_start_pick - self.t_relative_back
