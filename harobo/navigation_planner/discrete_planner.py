...
class DiscretePlanner:
    ...
    def plan(
        ...
        if self.last_action == DiscreteNavigationAction.MOVE_FORWARD or (
            ...
            # not checking collision when getting out of stuck
            if not self.last_step_getting_out:
        ...
        if not found_goal and stop:
            print("Reached ur goal. Replan a new one")
            reach_hgoal = True
